...
describe("ResultsAggregator Contract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should set correct candidate manager reference", async function () {
      expect(await resultsAggregator.candidateManager()).to.equal(await candidateManager.getAddress());
    });

    it("Should authorize the owner by default", async function () {
      expect(await resultsAggregator.authorizedContracts(owner.address)).to.be.true;
    });
  });
  ...
  describe("Authorization", function () {
    it("Should allow owner to authorize contracts", async function () {
      await resultsAggregator.authorizeContract(authorizedContract.address);
      expect(await resultsAggregator.authorizedContracts(authorizedContract.address)).to.be.true;
    });

    it("Should allow owner to revoke authorization", async function () {
      await resultsAggregator.authorizeContract(authorizedContract.address);
      await resultsAggregator.revokeContractAuthorization(authorizedContract.address);
      expect(await resultsAggregator.authorizedContracts(authorizedContract.address)).to.be.false;
    });
  ...
  describe("Winner Detection", function () {
    ...
    it("Should return candidate with most votes as winner", async function () {
      ...
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(1, 3);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(2, 2);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(3, 1);
    ...
    it("Should handle ties correctly", async function () {
      ...
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(1, 2);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(2, 2);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(3, 1);
  ...
  describe("Candidates by Vote Count", function () {
    it("Should return candidates sorted by vote count (descending)", async function () {
      ...
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(1, 1);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(2, 3);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(3, 2);
    ...
    it("Should handle equal vote counts correctly", async function () {
      ...
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(1, 1);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(2, 1);
      await candidateManager.connect(authorizedContract).updateCandidateVoteCount(3, 1);
